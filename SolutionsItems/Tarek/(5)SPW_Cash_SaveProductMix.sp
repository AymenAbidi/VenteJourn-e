create or alter procedure "SPW_Cash_SaveProductMix" (
    BUSINESSDATE date)
as
...
BEGIN
  /*************************************
   * Mise ? jour du PMX ? la journ?e   *
   *************************************/
  /* Suppression des donn?es existantes */
  DELETE FROM CSHPMXJOUR WHERE CAL_DTACTIVITE = :BusinessDate;

  /* R?cup?ration des Ventes, Pomos, Repas Employ?s/Managers et Pertes ? partir du POS */
  INSERT INTO CSHPMXJOUR(CAL_DTACTIVITE,
                         PRV_COD,
                         PVJ_PRIXBRUTSP,
                         PVJ_PRIXBRUTEMP,
                         PVJ_PRIXNETSP,
                  ...
                  SELECT :BusinessDate,
                         PMX.PRODUCT_CODE,
                         CAST(IIF(PRO.EAT_IN_PRICE = 0, 0, PRI.EAT_IN_PRICE) AS NUMERIC(6,2)),
                         CAST(IIF(PRO.TAKE_OUT_PRICE = 0, 0, PRI.TAKE_OUT_PRICE) AS NUMERIC(6,2)),
                         CAST(IIF(PRO.EAT_IN_PRICE = 0, 0, PRI.NET_EAT_IN_PRICE) AS NUMERIC(6,2)),
                         CAST(IIF(PRO.TAKE_OUT_PRICE = 0, 0, PRI.NET_TAKE_OUT_PRICE) AS NUMERIC(6,2)),
                         PMX.SOLD_EAT_IN_QY,
                         PMX.SOLD_TAKE_OUT_QY,
                         PMX.PROMO_EAT_IN_QY + PMX.PROMO_TAKE_OUT_QY,
                         PMX.WASTE_EAT_IN_QY + PMX.WASTE_TAKE_OUT_QY + COALESCE(PER.PRP_TOTQTEPERTES, 0),
                         PMX.CREW_MEAL_EAT_IN_QY + PMX.CREW_MEAL_TAKE_OUT_QY + COALESCE(PER.PRP_TOTQTEREPASEQP, 0),
  ...
  /* R?cup?ration des Repas Employ?s/Managers et Pertes ? partir de SIR pour les produits non vendus */
  INSERT INTO CSHPMXJOUR(CAL_DTACTIVITE,
                         PRV_COD,
                         PVJ_PRIXBRUTSP,
                         PVJ_PRIXBRUTEMP,
                  ...
                  SELECT :BusinessDate,
                         ...
                         0,     /* PVJ_QTEEMP */
                         0,     /* PVJ_QTEPROMOS */
                         COALESCE(PER.PRP_TOTQTEPERTES, 0),
                         COALESCE(PER.PRP_TOTQTEREPASEQP, 0),
                         COALESCE(PER.PRP_TOTQTEREPASMGR, 0),
                         0,     /* PVJ_MTREPASEMP */
                         0,     /* PVJ_QTEDISCOUNTSP */
         ...
         LEFT OUTER JOIN VW_CMU_DAILY_PMX PMX ON PMX.PRODUCT_CODE = PER.PRV_COD
                   WHERE PMX.PRODUCT_CODE IS NULL;

  /*************************************
   * Mise ? jour du PMX au 1/4 d'heure *
   *************************************/
  DELETE FROM CSHPMXHOR WHERE CAL_DTACTIVITE = :BusinessDate;

  /* R?cup?ration des Ventes totales */
  INSERT INTO CSHPMXHOR(CAL_DTACTIVITE,
                        PRV_COD,
                        PHR_HR,
                        PHR_VENDUSP,
                        PHR_VENDUEMP,
  ...
  /* Mise ? jour des Ventes Comptoir (FC) */
  FOR
    SELECT PRODUCT_CODE,
           QUARTER_TIME,
           SOLD_EAT_IN_QY,
  ...
  BEGIN
    UPDATE CSHPMXHOR
       SET PMX_FRC_EAT_IN_SOLD_QY = :SoldEatInQy,
     ...
     WHERE (CAL_DTACTIVITE = :BusinessDate)
       AND (PHR_HR = :QuarterTime)
       AND (PRV_COD = :ProductCode);
  END

  /* Mise ? jour des Ventes Drive (DT) */
  FOR
    SELECT PRODUCT_CODE,
           QUARTER_TIME,
           SOLD_EAT_IN_QY,
  ...
  BEGIN
    UPDATE CSHPMXHOR
       SET PHR_VENDUDRV = :SoldTakeOutQy,
  ...
  /* Mise ? jour des Ventes Kiosk (CSO) */
  FOR
    SELECT PRODUCT_CODE,
           QUARTER_TIME,
           SOLD_EAT_IN_QY,
  ...
  BEGIN
    UPDATE CSHPMXHOR
       SET PMX_KSK_SOLD_QY_IN = :SoldEatInQy,
  ...
  /* Mise ? jour des Ventes McCaf? (MCC) */
  FOR
    SELECT PRODUCT_CODE,
           QUARTER_TIME,
           SOLD_EAT_IN_QY,
  ...
  BEGIN
    UPDATE CSHPMXHOR
       SET PMX_MCF_SOLD_QY_IN = :SoldEatInQy,
  ...
  /* Mise ? jour des Ventes Web (DLV + CK) */
  FOR
    SELECT PRODUCT_CODE,
           QUARTER_TIME,
           SUM(SOLD_EAT_IN_QY),
           ...
           SUM(TOTAL_TAX_AM)
      FROM VW_CMU_QUARTER_HOUR_PMX
     WHERE (POD_TYPE = 'DLV' OR POD_TYPE = 'CK')
       AND (SOLD_EAT_IN_QY <> 0     OR SOLD_TAKE_OUT_QY <> 0 OR
            PROMO_EAT_IN_QY <> 0    OR PROMO_TAKE_OUT_QY <> 0 OR
            DISCOUNT_EAT_IN_QY <> 0 OR DISCOUNT_TAKE_OUT_QY <> 0)
  ...
  BEGIN
    UPDATE CSHPMXHOR
       SET PMX_WEB_EAT_IN_SOLD_QY = :SoldEatInQy,
