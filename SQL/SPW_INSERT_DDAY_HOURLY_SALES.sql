CREATE PROCEDURE [dbo].[SPW_INSERT_DDAY_HOURLY_SALES]
    @list APP_DDAY_HOURLY_SALES_TYPE READONLY,
    @business_date datetime
AS
BEGIN
    DELETE FROM APP_DDAY_HOURLY_SALES WHERE DDHS_BUSINESS_DT=convert(varchar, @business_date, 101) and DDHS_SITE_ID =(select distinct DDHS_SITE_ID from @list)
    INSERT INTO APP_DDAY_HOURLY_SALES (DDHS_SITE_ID,
DDHS_BUSINESS_DT,
DDHS_SALES_TM,
DDHS_MCDE_SIR_ID,
DDHS_MVAL_SIR_ID,
DDHS_LLVR_SIR_ID,
DDHS_SALES_PROD_AM,
DDHS_SALES_NON_PROD_AM,
DDHS_EAT_IN_TAC_QY,
DDHS_TAKE_OUT_TAC_QY,
DDHS_EAT_IN_SALES_AM,
DDHS_TAKE_OUT_SALES_AM,
DDHS_DISCOUNT_IN_TAC_QY,
DDHS_DISCOUNT_OUT_TAC_QY,
DDHS_DISCOUNT_IN_SALES_AM,
DDHS_DISCOUNT_OUT_SALES_AM,
DDHS_CREW_HOURS_WORKED,
DDHS_PROCESS_DT)
    SELECT DDHS_SITE_ID,
convert(varchar, @business_date, 101),
DDHS_SALES_TM,
DDHS_MCDE_SIR_ID,
DDHS_MVAL_SIR_ID,
DDHS_LLVR_SIR_ID,
DDHS_SALES_PROD_AM/1000,
DDHS_SALES_NON_PROD_AM/1000,
DDHS_EAT_IN_TAC_QY,
DDHS_TAKE_OUT_TAC_QY,
DDHS_EAT_IN_SALES_AM/1000,
DDHS_TAKE_OUT_SALES_AM/1000,
DDHS_DISCOUNT_IN_TAC_QY,
DDHS_DISCOUNT_OUT_TAC_QY,
DDHS_DISCOUNT_IN_SALES_AM/1000,
DDHS_DISCOUNT_OUT_SALES_AM/1000,
DDHS_CREW_HOURS_WORKED,
convert(varchar, getdate(), 121)
    FROM @list;
    
END
